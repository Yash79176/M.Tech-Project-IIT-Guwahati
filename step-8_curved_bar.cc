...
const double Tref = 315.; 			// Kelvin (reference temp of SMA)
const double Tmax = 315.;			// Kelvin (max temp)
const double Pmax = 2000e6;			// MPa  (max. stress)
...
template <int dim>
inline void NeumannBoundary<dim>::vector_value(const Point<dim> &p /*p*/, Vector<double> &values) const
{
	AssertDimension(values.size(), dim);
	  	//const double p0   = 400.e6;				// N /per unit length
	  	//const double total_time =2.;
	  //	const Point<dim> main_center (0., 0., 0.);
	  //	const double angle = std::atan((p(1)-main_center(1))/(p(0)-main_center(0)));
	  	const double Pmax = 2000e9;
	  	const double angle = std::atan2(p(1),p(0));
	  //	const double Rm = 0.00283
	  	values    = 0;
	  	if (present_time <= 1.)
	  	{
	  //		values(0) = Pmax *((present_time - 0.)/(1.-0.));			// total time is 1s
	  		values(0) = Pmax * sin(angle) *((present_time - 0.)/(1.-0.)) ;
	  		values(1) = Pmax * cos(angle) *((present_time - 0.)/(1.-0.));
	  		values(2) = 0;
	  	}
	  	else if (present_time <= 2.)
	  	{
	  //		values(0) = Pmax *((2.0 - present_time)/(2. - 1.));
	  		values(0) = Pmax*((2.0 - present_time)/(2. - 1.)) * sin(angle);
	  		values(1) = Pmax*((2.0 - present_time)/(2. - 1.)) * cos(angle);
	  		values(2) = 0;
	  	}
	  	else
	  	{
...
void FEAnalysis<dim>::declare_parameters(ParameterHandler &prm)
{
	...
	prm.declare_entry("Pmax", "2000e6", Patterns::Double(), " maximum load ");
	...
	prm.declare_entry("Af_0", "315", Patterns::Double(), " Reverse transformation finish temperature");
	prm.declare_entry("As_0", "295", Patterns::Double(), " Reverse transformation start temperature ");
	prm.declare_entry("Ms_0", "291", Patterns::Double(), "Forward transformation start temperature");
	prm.declare_entry("Mf_0", "271", Patterns::Double(), "Forward transformation finish temperature");
	prm.declare_entry("c_M", "7e6", Patterns::Double(), " slope of forward transformation zone ");
	prm.declare_entry("c_A", "7e6", Patterns::Double(), " slope of reverse transformation zone ");
	prm.declare_entry("Hmax", "0.01", Patterns::Double(), "Max. transformation strain");
	prm.declare_entry("E_M", "70e9", Patterns::Double(), "modulus in martensite phase ");
	prm.declare_entry("E_A", "30e9", Patterns::Double(), "modulus in austenite phase ");
...
{
    const double inner_radius = 0.0025, outer_radius = 0.00317;
    GridGenerator::cylinder_shell(triangulation, 0.082, inner_radius, outer_radius, 16, 40);
    for(typename Triangulation<dim>::active_cell_iterator
    	...
    	for (unsigned int f = 0; f < GeometryInfo <dim> :: faces_per_cell; ++f)
    		...
    		{
    			if (std::fabs(cell->face(f)->center()[2]) - 0 < 1e-6)
    				{
    				cell->face(f)->set_boundary_id (1);
    				std::cout<<"boundary_id 1 set"<<std::endl;
    				}
    			else if(std::fabs(cell->face(f)->center()[2] - 0.082) < 1e-6){
    				cell->face(f)->set_boundary_id (3);
    				std::cout << "center="  << cell->face(f)->center()[2] <<std::endl;
...
//		for (const auto &face : cell->face_iterators())
        for (unsigned int f=0; f <GeometryInfo<dim>::faces_per_cell; ++f)
			if (cell->face(f)->at_boundary() && (cell->face(f)->boundary_id() == 3))
			{
				...
				vec.resize (fe_face_values.get_quadrature_points().size());
				for(unsigned int p = 0; p < vec.size(); ++p)
				    		   {
				    			   vec[p] = cell->face(f)->center();
				    		   }
				//for (const unsigned int q_point : fe_face_values.quadrature_point_indices())
				for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
				{
					neumann_boundary.vector_value_list(vec/*fe_face_values.get_quadrature_points()*/, neumann_values);
//					for (const unsigned int i : fe_values.dof_indices())
			          for (unsigned int i=0; i<dofs_per_cell; ++i)
					{
						const unsigned int component_i = fe.system_to_component_index(i).first;
